...
웹캠 영상에서 얼굴·객체를 실시간으로 탐지하고, 화면 내 **위치**, **크기(비중)**, **고개 방향**을 계산한 뒤 한국어 TTS로 안내합니다.

---

## 프로젝트 목적

BITAmin(비타민) 빅데이터 동아리 프로젝트로, 시각장애인이 셀프 카메라로 사진을 찍을 때 **"피사체가 화면 어디에 있는지"** 를 듣고 촬영 구도를 맞출 수 있도록 돕는 것이 목표입니다.
...
| **4. 음성 안내** | pyttsx3 TTS — *"현재 얼굴 비중은 23%이고 위치는 가운데입니다."* |
...
> \* RetinaNet / MobileNet-SSD / YOLOX는 `notebooks/training/`에서 비교 실험 수준입니다. **실시간 추론의 메인 파이프라인은 YOLOv8** 입니다.
...
커스텀 얼굴 모델은 직접 학습하거나, `notebooks/training/yolov8/runs/`의 학습 결과 가중치를 `models/face_best.pt`로 복사합니다.
...
> **레거시 폴더** (`yolov8/`, `headPose/`, `custom_dataset*/`, `test/`, `dataset/`)은 이전 구조의 잔여물입니다. `src/`로 이전 완료 후 삭제 가능합니다. 각 폴더에 `DEPRECATED.md` 참고.
...
- **소속**: BITAmin 13기 (빅데이터 분석 연합동아리)
- **목표**: OpenCV + Object Detection 전이학습으로 촬영 latency 최소화 및 정확도 향상
- **데이터**: K-FACE(한국인 얼굴) + Roboflow 공개 데이터 — **비상업·학습 목적**

---

## 알려진 이슈 & 개선 방향

- [ ] `models/face_best.pt` — 학습 가중치를 repo에 포함하거나 Release로 배포
- [ ] RetinaNet/MobileNet/YOLOX 실험 결과를 실시간 파이프라인과 통합
- [ ] 레거시 폴더 (`yolov8/`, `custom_dataset*/`) 정리
- [ ] Android/iOS 네이티브 앱 연동

---

## 라이선스 & 데이터 출처

- Roboflow face-detection dataset: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
- K-FACE: 한국과학기술연구원 / 한국지능정보사회진흥원 (활용계획서 준수)

---

## 팀

BITAmin 13기 — Camera Object Detection Project
